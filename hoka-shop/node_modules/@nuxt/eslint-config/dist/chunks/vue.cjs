...
const flat = require('../shared/eslint-config.27361737.cjs');
...
async function vue(options) {
  ...
  const configs = [
    ...
    },
    {
      ...
      processor: options.features?.formatters ? eslintMergeProcessors.mergeProcessors([
  ...
  for (const config of configs) {
    if (config.rules)
      config.rules = flat.removeUndefined(config.rules);
  }
  return configs;
}
